...
# D5 ARCHITECTURE ADDENDUM — WORKSPACE RAIL + CONTEXTUAL SIDEBAR

## 1. Core navigation architecture

Wamiro has two navigation layers:

```text
RAIL
= major workspace / product-domain switcher

SIDEBAR
= features inside the selected workspace
```
...
The sidebar is **workspace-aware, not global**.

## 2. Why this architecture matters

Do not build one permanent sidebar containing every Wamiro capability.
...
which increases cognitive load and destroys workspace identity.
...
Rail
↓
Select major workspace
↓
Load that workspace's navigation
↓
Show only its relevant features
```

## 3. Rail = major mental models

A rail item must represent a meaningful product domain.
...
Do not promote every feature into the rail.

Examples that should normally remain inside People:
...
Examples that should normally remain inside Work:
...
When additional workspaces make the rail crowded, provide:
...
The launcher shows only workspaces the user can access.
...
Pin workspace
Unpin workspace
Reorder pinned workspaces
```

Do not allow users to dismantle critical navigation.

## 5. Workspace-aware sidebar

Visible sidebar should be derived from:
...
Create a centralized model such as:
...
Do not duplicate sidebar definitions in page components.

## 7. Workspace-specific commands

The global `Ctrl/Cmd + K` command palette stays the same component, but commands become contextual.
...
## 8. Workspace-specific search

Global search remains global.

Contextual search scopes:
...
Reuse the same search component and interaction model.

## 9. Workspace switch behavior

When switching the rail:

```text
1. Update active rail state
...
8. Update commands
9. Update shortcuts
10. Navigate to the workspace default page
```

Do not leave stale navigation from the previous workspace.
...
Reset stale state on:

```text
tenant change
permission change
invalidated data context
explicit reset
```

## 11. Cross-workspace navigation
...
Do not duplicate cross-domain objects into every sidebar.
...
+
Workspace navigation sheet
+
Content
```

The user must always know which major workspace is active.
...
Need a new feature?
↓
Does it belong to an existing workspace?
    Yes → add to that workspace sidebar.
    No  → evaluate whether it is a genuinely new mental model.
              ↓
           New rail workspace only if justified.
```

This prevents navigation sprawl as Wamiro grows.
...
WamiroAppShell
WamiroRail
WamiroWorkspaceSwitcher
WamiroSidebar
WamiroSidebarSection
WamiroSidebarItem
```

This architecture becomes mandatory for D6 onward.
